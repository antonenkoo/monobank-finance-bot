...
_CHANGELOG: dict[str, str] = {
    ...
    "v1.5": (
        ...
        "✨ <b>Новые функции:</b>\n"
        "• ⚡ Статистика открывается мгновенно — данные кэшируются и обновляются автоматически после каждой транзакции\n"
        "• 📆 В статистике появился показатель «сколько осталось тратить в день до конца месяца» — по каждой категории и по общему бюджету\n"
        "• 🚨 Если лимит превышен — вместо дневной суммы отображается «Бюджет превышен»\n"
        "• 📅 Команда /report — получи PDF-отчёт за любой месяц через меню выбора\n"
        "• 📊 PDF-отчёт содержит: круговую диаграмму расходов по категориям, столбчатый график, график расходов по дням с накопительной линией и полный список транзакций\n\n"
        "🐛 <b>Исправления:</b>\n"
        "• 🕐 При ручном добавлении транзакции время отображалось в UTC вместо местного — исправлено\n"
        "• 📄 В PDF-отчёте не отображались названия категорий — исправлено\n"
        "• 💬 Фидбеки, отправленные через бота, больше не пропадают после перезапуска"
    ),
}
